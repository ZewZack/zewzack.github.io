...
function displayNotes(){
		...
		html += `
				<div class="card mx-4 my-2 bg-dark text-white thatsMyNote" style="width: 18rem;">
					<div class="card-body">
						...
						<button id="${index}" onclick=deleteNote(this.id) class="btn btn-danger">sil (silmeyin la)</button>
					</div>
				</div>
			`;
	...
	if(notesObj.length != 0){
		...
		noteEle.innerHTML = '<h3 style="text-align: center; color: grey;">olm silmeyin şu notları yaw :/</h3>';
	}
...
{
  var result1 = confirm("emin misin?");
  if (result1) {
    var result2 = confirm("bu notlar zewzeğin notları değil demi?");
    if (result2) {
      var result3 = confirm("niyeyse inanamadım?")
      if (result3) {
        var result4 = confirm("lütfen zewzekin notuysa iptale basar mısın knk")
        if (result4) {
          var result5 = confirm("basmıyon korkuyom :(")
          if (result5) {
            var result6 = confirm("böyle uğraşmak da zor oluyodur şimdi xdd")
            if (result6) {
              var result7 = confirm("hadi sen benimkini silme ben de bu işsizliği bitireyim nası anlaştık mı?")
              if (result7) {
                var result8 = confirm("o zaman anlaştık xd. eğer zamanını aldıysam kb :D")
                if (result8) {
                                let notesObj;
	                              let notesString = localStorage.getItem('notes');
	...
	Array.from(cards).forEach(function(ele){
		...
		if(cardText.includes(inputText)){
			...
			if(countNone === cards.length){
				document.getElementById('noMatches').innerHTML = '<h3 style="text-align: center; color: grey;">yanlış aradın knk xd</h3>';
			}
			else{
				document.getElementById('noMatches').innerHTML = '';
			}
